...
feat_arr = ['ASProbability', 'ASNumbers', 'ASDurations', 'Food', 'Water', 'Distance', 'ASFoodIntensity', 'ASWaterIntensity', 'ASLocomotionIntensity']
feat_arr_units = ['Active state probability', 'Number AS onsets', 'Total time [sec]', 'Food consumed [g]', 'Water consumed [mg]', 'Distance travelled [m]', 'ASFoodIntensity', 'ASWaterIntensity', 'ASLocomotionIntensity [cm/ASsec]']

all_data_name = 'data/all_features_mousedays_11bins.npy'
data_orig_master = np.load(all_data_name)  # 9 x 1921 x (3 labels + 11 feature time bins)

MOUSEDAYS = False

# Feature to study
AS_i = 0; AS = feat_arr[AS_i]
print "PCA analysis for feature %s" % feat_arr[AS_i]
features = data_orig_master[AS_i, :, 3:]
labels = data_orig_master[AS_i, :, 0:3]  # 1st col = strain, 2nd = mouse number, 3rd = day
if not MOUSEDAYS:
    mice = day_to_mouse_average(features, labels)  # 170 mice
else:
    mice = np.hstack([labels[:, :2], features])  # 1921 mousedays

# Determing linear map L such that vectors LF (F the 11-D feature vector) have identity covariance = I
mouse_mz = mice[:, 2:13] - mice[:, 2:13].mean(axis=0)
Cmz = np.dot(mouse_mz.T, mouse_mz) / mouse_mz.shape[0]  # covariance matrix
d, v = np.linalg.eig(Cmz)   # eigenvalues and eigenvectors (PCA components) all v[i] norm 1
idx = np.argsort(d)
...
projected_features_3d = np.column_stack((mice[:, 0:2], projected_features[:, :3]))
Cov_LF = np.dot(projected_features.T, projected_features) / projected_features.shape[0]
I = np.identity(Cov_LF.shape[0])  # check covariance of new features is identity
...
plt.figure()
plt.clf()    
plt.plot(first, lw=5, label='1$^{st}$ principal component: %d%% variance captured' % (100 * variances_captured[1]))
...
plt.yticks(np.arange(0, len(strains), 2), np.arange(1, len(strains) + 1, 2), fontsize=9)
plt.xticks(np.arange(0, 11, 2), np.arange(1, 12, 2), fontsize=12)
